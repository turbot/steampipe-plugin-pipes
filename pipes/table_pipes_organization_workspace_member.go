...
func tablePipesOrganizationWorkspaceMember(_ context.Context) *plugin.Table {
	return &plugin.Table{
		Name:        "pipes_organization_workspace_member",
		...
		Columns: []*plugin.Column{
			...
			{
				Name:        "created_by",
				Description: "Information about the user who added the member.",
				Type:        proto.ColumnType_JSON,
			},
			{
				Name:        "updated_at",
...
func getOrganizationWorkspaceMember(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	orgHandle := d.EqualsQuals["org_handle"].GetStringValue()
